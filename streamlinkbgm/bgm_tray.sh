...
MENU="Next Song!$SENDER next|Previous Song!$SENDER prev|Pause/Resume!$SENDER toggle|Shuffle!$SENDER shuffle|---|Seek +10s!$SENDER seekf|Seek -10s!$SENDER seekb|Volume +5!$SENDER volup|Volume -5!$SENDER voldn|---|Choose BGM URL...!bash -lc 'choose_bgm_url'|Set BGM URL...!bash -lc 'URL=\$(yad --entry --width=700 --title=\"Set BGM URL\" --text=\"Enter YouTube or playlist URL:\"); [ -n \"\$URL\" ] && \"\$BGM_CTL\" seturl \"\$URL\"'|Delete BGM URL...!bash -lc 'URL=\$(yad --entry --width=700 --title=\"Delete BGM URL\" --text=\"Enter URL to remove (exact match in sources file):\"); [ -n \"\$URL\" ] && \"\$BGM_CTL\" delurl \"\$URL\"'|Clean duplicate URLs!bash -lc '\"$BGM_CTL\" cleandupes'|Edit BGM URL list...!bash -lc 'if command -v xfce4-terminal >/dev/null 2>&1; then xfce4-terminal --maximize --title=\"BGM URLs\" --command=\"nano ~/.config/streamlink_bgm_sources\"; else x-terminal-emulator -e nano ~/.config/streamlink_bgm_sources; fi'|---|Open in Browser!$SENDER open|Quit Tray!bash -lc 'kill $$'"
